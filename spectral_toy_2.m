...
dimW = 500;
num_blocks = 1;  % You'll actually get num_blocks + 1 blocks
...
error_links = 1000; % Number of sparse interconnections to add
missing_links = 40000; % Number of links to remove from the blocks
H = triu(W,1);
G = tril(10*ones(dimW));
H = H + G;
ind = find(H == 0);
...
subplot(1,2,1)
plot(Y(:,1));
title('first eigenvector (noisy)')
subplot(1,2,2)
plot(Y(:,2));
title('second eigenvector (noisy)')
